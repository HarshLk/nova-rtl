...
from nova_rtl.contracts.base import NonNegativeInt, StrictContract, UtcDatetime, canonical_sha256
...
class RecoveryEnvelope(StrictContract):
...
def _id(prefix: str, payload: object) -> str:
    digest = canonical_sha256({"payload": payload}).removeprefix("sha256:")[:20]
...
def _select_action(
    ...
    if getattr(failure, "repairability", None) == "HUMAN_REVIEW" or (
        not getattr(failure, "retryable", True)
        and DEFAULT_ACTION[failure.failure_family] not in TERMINAL_ACTIONS
    ):
        if "STOP_RUN_OR_REQUEST_HUMAN" in allowed:
            return "STOP_RUN_OR_REQUEST_HUMAN"
    ...
    if budgets.remaining_family_budget == 0 and ESCALATION_LEVEL[selected] <= 3:
        candidates = [
            item
            for item in allowed
            if ESCALATION_LEVEL[item] >= ESCALATION_LEVEL["OPPORTUNITY_REANALYSIS"]
            and item not in TERMINAL_ACTIONS
        ]
...
def route_recovery_envelope(
    ...
    *,
    policy: RecoveryPolicyRegistry,
    candidate_failure_fingerprint_ids: tuple[str, ...] = (),
) -> RecoveryEnvelope:
    """Build and validate the complete deterministic recovery authority envelope."""
    ...
    authorized_evidence_ids = tuple(
        sorted(
            {
                item.evidence_id
                for item in (*failure.primary_evidence_refs, *directive.evidence_refs)
            }
        )
    )
    request_payload = {
        "schema_version": 1,
        "recovery_request_id": _id(
            "recovery_request", (plan.route_plan_hash, failure.failure_event_id)
        ),
        "failure_event_id": failure.failure_event_id,
        ...
        "recovery_route_plan_hash": plan.route_plan_hash,
        "route_plan": plan,
        "candidate_lineage_ids": lineage,
        "candidate_failure_fingerprint_ids": tuple(
            dict.fromkeys(candidate_failure_fingerprint_ids)
        ),
        "allowed_actions": plan.allowed_actions,
        "excluded_transform_families": excluded,
        "eligible_roles": plan.eligible_roles,
    ...
    provisional_request = RecoveryRequest.model_construct(
        **request_payload, request_hash="sha256:" + "0" * 64
    )
    request = RecoveryRequest(
        **request_payload,
        request_hash=canonical_sha256(
            provisional_request, exclude=frozenset({"request_hash"})
        ),
    )
    decrement_lineage = action not in TERMINAL_ACTIONS and action != "RETRY_INFRASTRUCTURE"
    decrement_family = action not in TERMINAL_ACTIONS
    decision_payload = {
        "schema_version": 1,
        "recovery_decision_id": _id("recovery_decision", (request.request_hash, action)),
        "failure_event_id": failure.failure_event_id,
    ...
    return RecoveryEnvelope(route_plan=plan, request=request, decision=decision)
